...
for i=1:6
    ...
    if i==1||i==2||i==3           %同步态的三条腿/同时着地的三条腿
        ...
        for j=(nn/4+1):nn/2       %从s的中间开始，半个s步态
            trace_worldCo{i}(:,j-nn/4)=baseBias{i}+[xs(j);0;zs(j)];% (Δx; 0; Δz)
        end
        for j=1:nn/2              %半个s后接一个p步态
            trace_worldCo{i}(:,nn/4+j)=baseBias{i}+[xp(j);0;zp(j)];
        end
        for j=1:nn/4              %再接剩下半个s  后半s--p--前半s
            trace_worldCo{i}(:,j+(3*nn/4))=baseBias{i}+[xs(j);0;zs(j)];
        end

    else    %i==4||i==5||i==6      另外三条s和p步态反过来

        for j=(nn/4+1):nn/2   
            trace_worldCo{i}(:,j-nn/4)=baseBias{i}+[xp(j);0;zp(j)];%xp和xs顺序交换
        end
        for j=1:nn/2   
            trace_worldCo{i}(:,nn/4+j)=baseBias{i}+[xs(j);0;zs(j)];
        end
        for j=1:nn/4              %后半p--s--前半p
            trace_worldCo{i}(:,j+(3*nn/4))=baseBias{i}+[xp(j);0;zp(j)];
        end
    end
end
